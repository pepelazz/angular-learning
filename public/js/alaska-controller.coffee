...
emailRouterConfig = ($routeProvider, $locationProvider) ->
  $routeProvider.
  when('/',{
  ...
  when('/view/:id',{
    ...
    templateUrl: '/public/tmpl/detail.html'
  }).
  otherwise({
    redirectTo: '/'
  })
...
myApp.config emailRouterConfig
...
myApp.controller 'ListController', ($scope, $http, $log) ->
  $http.get('/messages').success((data, status, headers, config) ->
    $scope.messages = data
